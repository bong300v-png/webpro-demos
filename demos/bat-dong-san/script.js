/**
 * DEMO: Bat Dong San — LuxEstate
 * Script: Navbar scroll, scroll reveal, counter, modal
 */

// Modal functions
function openModal(t) { document.getElementById('authModal').classList.add('open'); switchTab(t || 'login') }
function closeModal() { document.getElementById('authModal').classList.remove('open') }
function switchTab(t) { document.querySelectorAll('.estate-tab').forEach((b, i) => b.classList.toggle('active', (t === 'login' && i === 0) || (t === 'register' && i === 1))); var lf = document.getElementById('loginForm'); var rf = document.getElementById('registerForm'); if (lf) lf.classList.toggle('hidden', t !== 'login'); if (rf) rf.classList.toggle('hidden', t !== 'register') }
...
// Scroll reveal
const revealEls = document.querySelectorAll('.listing-card, .feature-card, .stat-item, .testimonial-card, .cta__content');
