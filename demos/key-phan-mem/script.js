/** Key Phan Mem — KeyVault: Auth modal + toast + reveal */
...
document.querySelectorAll('.prod-card,.step,.cta-box').forEach(el => { el.classList.add('reveal'); new IntersectionObserver(([e]) => { if (e.isIntersecting) e.target.classList.add('visible') }, { threshold: .15 }).observe(el) });
