/* BloomDecor — Script: modal tabs, toast, scroll reveal */
...
document.querySelectorAll('.pkg-card,.gallery-item,.booking-box').forEach(el => { el.classList.add('reveal'); new IntersectionObserver(([e]) => { if (e.isIntersecting) e.target.classList.add('visible') }, { threshold: .15 }).observe(el) });
