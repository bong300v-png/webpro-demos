/* Artisan — Script: modal, toast, scroll reveal */
...
document.querySelectorAll('.product-card,.value,.showcase-item').forEach(el => { el.classList.add('reveal'); new IntersectionObserver(([e]) => { if (e.isIntersecting) e.target.classList.add('visible') }, { threshold: .15 }).observe(el) });
